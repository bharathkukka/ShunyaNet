"""
Softmax (Multinomial Logistic Regression) Classifier on Digits Dataset
This script trains and evaluates a Softmax classifier on the Digits dataset.
"""
import numpy as np
...
def main(C=1.0, test_size=0.3, random_state=42, normalize=False, save_results=None, save_plot=None, verbose=True):
    ...
    print("Feature names:")
    print(digits.feature_names if hasattr(digits, 'feature_names') else [f'col_{i}' for i in range(X.shape[1])])
    print("Features:")
    print(X[:5, :])
    print("Labels:")
    print(y[:5])
    ...
    # Define Softmax classifier (multinomial logistic regression)
    clf = LogisticRegression(C=C, solver='lbfgs', multi_class='multinomial', max_iter=1000, random_state=random_state)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Softmax Classifier on Digits Dataset")
    ...
    main(C=args.C, test_size=args.test_size, random_state=args.random_state,
